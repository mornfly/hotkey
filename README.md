...
2 推送性能：对外推送目前性能约平稳推送每秒10-12万次，譬如有1千台server，一台worker上每秒产生了100个热key，那么这1秒会平稳推送100 * 1000 = 10万次，10万次推送会明确在1s内全部送达。每秒推送20万次，会出现1s的延迟，实际线上出现过单机一秒48万次推送任务的非常规场景，个别key送达client端出现了超过5秒的延迟。（注：延迟是指从热key产生时的时间戳和client端收到该key时的时间戳的差值）
